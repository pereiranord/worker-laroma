...
async function atRetry(env,path,opts={},tries=2){
  for(let i=0;i<tries;i++){
    try{const r=await airtable(env,path,opts);if(!r.data.error)return r;}
...
export default{
  async fetch(request,env){
    ...
    // GET /menu — 7 tabelas em paralelo, filtra Ativo=true, exclui Pizzas Médias
    if(path==='/menu'&&method==='GET'){
      const tables=['Pizzas Grandes','Brotinhos','Esfirras','Bebidas','Variações de Bebidas','Adicionais','Bordas'];
      try{
        const results=await Promise.all(tables.map(t=>atRetry(env,encodeURIComponent(t)).then(({data,status})=>({t,data,status}))));
        const menu={};
        for(const{t,data,status}of results){
          if(data.error||!data.records)return E(`Erro na tabela "${t}"`,500,{airtable:data.error,status});
    ...
    // POST /auth/google — valida Firebase, busca/cria Clientes
    ...
    // GET /cupom/validar?codigo= — valida cupom pelo código
    ...
    // GET /meus-pedidos?uid= — apenas pedidos do usuário
    ...
    // GET /status-pedido/:id?uid= — polling leve
    ...
    // PATCH /pedidos/:id (admin) — muda status + timestamps automáticos
